...
**Body Parts** (21 cards), **Months** (12 cards, with zodiac etymology), and
**Clock / นาฬิกา** (the Thai six-hour time-telling system). No build step, no
framework, no CDNs at runtime — scripts, styles, and fonts are all served from
the repo itself.
...
| **Clock** (นาฬิกา) | 38 | clock→thai · thai→clock · quiz | clock→thai |
...
- `#naalika` — clock deck, *see clock → say time* (the core skill)
- `#naalika/thai` — *read Thai → set the clock* (reverse direction)
- `#naalika/mc` — multiple-choice quiz with same-period distractors + rule reveal

### Clock — drilling a conversion system, not a word list

Thai time isn't vocabulary, it's a conversion: the number you say depends on which of the day's periods the hour falls in, and it rarely matches the clock face (the face shows 7, the mouth says หนึ่งทุ่ม). Each card draws a real analog clock with hands computed from the time — `(h%12)*30 + m*0.5` for the hour hand — and drills the mapping `clock ↔ spoken Thai` both directions.

The seven periods, each with its own counting rule:

| Period | Range | Pattern |
|--------|-------|---------|
| ตี (tii) | 1–5 AM | ตี + number |
| เช้า (cháao) | 6–11 AM | number + โมงเช้า |
| เที่ยง (thîaŋ) | 12 PM | เที่ยง |
| บ่าย (bàai) | 1–3 PM | บ่าย + number + โมง (บ่ายโมง drops the number for 1 PM) |
| เย็น (yen) | 4–6 PM | number + โมงเย็น |
| ทุ่ม (thûm) | 7–11 PM | number + ทุ่ม — **the count restarts** (7 PM = หนึ่งทุ่ม) |
| เที่ยงคืน (thîaŋ-kʉʉn) | 12 AM | เที่ยงคืน |

The whole bank is generated by `makeTime(h, m)` from the 24-hour hour, so every card's period, clock angle and spoken phrase derive from one source of truth. Extras: a **Reference** cheat-sheet (the table above + the common traps), a **period-focus filter** to grind one period at a time, a **Thai/Arabic numeral switch** for the clock face (persisted), and a faint period tint behind the clock to anchor the time of day.
...
```
thai-cards/
├── index.html              # home screen + all deck card templates + modal
├── styles.css              # theme vars, home tiles, mode selector, motif layer
├── fonts.css               # @font-face declarations pointing at fonts/
├── clock.js                # drawClock(h, m, opts) → analog clock SVG (naalika)
├── data/
│   ├── body-parts.js       # WORDS + HIGHLIGHTS for the anatomical figure
│   ├── months.js           # 12 MONTHS with zodiac etymology
│   ├── times.js            # makeTime(h,m) generator + TIMES bank + PERIODS table
│   ├── zodiac-motifs.js    # MOTIFS + HOME_MARKS registries
...
- **Clock:** the bank is generated — edit the rules in `makeTime()` / `PERIODS` in `data/times.js`, or extend the seed list of times at the bottom of that file. `drawClock()` in `clock.js` is the single clock renderer (it takes `{ numerals: 'arabic' | 'thai' }`).
...
The zodiac watermarks on the Months deck (per-card per-month signs), the months home-tile mark, and the clock home-tile mark are from [Material Design Icons](https://materialdesignicons.com/) (`zodiac-*`, `calendar-clock`, `clock-outline`), licensed [Apache 2.0](https://github.com/Templarian/MaterialDesign/blob/master/LICENSE). The body-parts home-tile mark is a small hand-drawn standing figure. The analog clock faces on the Clock deck are drawn programmatically by `clock.js` (no icon asset). Each entry in `data/zodiac-motifs.js` carries a single-path SVG with `fill="currentColor"` so theme + opacity stay CSS-controlled (watermark renders dark in light theme, gold in dark theme, both at ~10% opacity).
